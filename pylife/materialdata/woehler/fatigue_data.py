...
class FatigueDataAccessor(signal.PylifeSignal):
    ...
    def basic_analysis(self, load_cycle_limit):
        self.load_cycle_limit = load_cycle_limit
        self.__data_sort()
        self.__slope()
        self.__pearl_chain_method()
        self.__analyzer = WoehlerCurveCreator(self)
        return self

    def _validate(self, obj, validator):
        validator.fail_if_key_missing(obj, ['load', 'cycles'])
    ...
    def __data_sort(self):
        ...
        self.load_min = self._obj.load.min()

        self.cycles_max = self._obj.cycles.max()
        self.cycles_min = self._obj.cycles.min()

        self.fractures = self._obj[self._obj.cycles < self.load_cycle_limit]
        self.runouts = self._obj[self._obj.cycles >= self.load_cycle_limit]

        self.__calc_ld_endur_zones()

        self.zone_inf_fractures = self.fractures[self.fractures.load < self.fatg_lim]

        self.ld_lvls = np.unique(self._obj.load, return_counts=True)
        self.ld_lvls_fin = np.unique(self.zone_fin.load, return_counts=True)
        self.ld_lvls_inf = np.unique(self.zone_inf.load, return_counts=True)
        self.ld_lvls_inf_frac = np.unique(self.zone_inf_fractures.load, return_counts=True)
    ...
    def __calc_ld_endur_zones(self):
        ...
        self.zone_fin = self.fractures[self.fractures.load > self.runouts.load.max()]
        zone_fin_min = self.zone_fin.load.min()
        if zone_fin_min == 0:
            self.fatg_lim = self.runouts.load.max()
        else:
            self.fatg_lim = np.mean([zone_fin_min, self.runouts.load.max()])
        self.zone_inf = self._obj[self._obj.load <= self.fatg_lim]

    def __slope(self):
        '# Computes the slope of the finite zone with the help of a linear regression function'

        self.a_wl, self.b_wl, _, _, _ = stats.linregress(np.log10(self.fractures.load),
    ...
    def __pearl_chain_method(self):
        ...
        '''
        # Mean load level:
        self.Sa_shift = np.mean(self.fractures.load)

        # Shift probes to the mean load level
        self.N_shift = self.fractures.cycles * ((self.Sa_shift/self.fractures.load)**(-self.k))
    ...
    def __collect_data_for_rossow_infinite(self):
        ...
        if len(self.ld_lvls_inf[0]) != len(self.ld_lvls_inf_frac[1]):
            x = {
                k: v for k, v in enumerate(~np.in1d(self.ld_lvls_inf[0], self.ld_lvls_inf_frac[0]))
                if v
            }
            if len([*x.keys()]) > 1:
    ...
    def initial_dict_bound(self):
        return {'SD_50': (-np.inf, self.fatg_lim*0.95),
                '1/TS': (0.5, 2),
                'k_1': (self.k*0.9, self.k*1.1),
                'ND_50': (self.N_E*0.8, self.load_cycle_limit*0.8),
                '1/TN': (self.TN*0.1, self.TN*10)}

    def woehler_probit(self):
        if self.__analyzer is None:
            self.basic_analysis(self._obj.cycles.max())
        return self.__analyzer.probit()

    def woehler_max_likelihood(self, fixed_parameters={}):
        if self.__analyzer is None:
            self.basic_analysis(self._obj.cycles.max())
        return self.__analyzer.max_likelihood_full(fixed_parameters)

    def woehler_max_likelihood_inf_limit(self):
        if self.__analyzer is None:
            self.basic_analysis(self._obj.cycles.max())
