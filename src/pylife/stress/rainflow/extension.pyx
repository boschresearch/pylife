...
def fourpoint_loop(double [::1] turns, long [::1] turns_index):
    ...
    cdef long [::1] from_index_v = from_index
    cdef long [::1] to_index_v = to_index

    residual_index = np.empty(len_turns, dtype=np.int64)

    cdef long [::1] residual_index_v = residual_index
