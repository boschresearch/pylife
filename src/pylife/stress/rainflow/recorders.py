...
class FullRecorder(LoopValueRecorder):
    ...
    def __init__(self):
        ...
        super().__init__()
        self._index_from = np.array([], dtype=np.int64)
        self._index_to = np.array([], dtype=np.int64)
    ...
    def record_index(self, index_from, index_to):
        """Record the index."""
        self._index_from = np.append(self._index_from, index_from)
        self._index_to = np.append(self._index_to, index_to)
