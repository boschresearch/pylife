...
class _Hookeslawcore:
    """Parent class for the multidimensional Hooke's Law implementation.
    Defines the properties and checks for correct inputs"""
    ...
    def __init__(self, E, nu):
        ...
        self._nu = nu
        self._G = E / (2.0 * (1 + nu))
        self._K = E / (3.0 * (1 - 2 * nu))

    def _validateinit(self, nu):
        """Validates the input of the Poisson\'s ratio"""
        if nu < -1 or nu > 1.0 / 2:
            raise ValueError(
                "Poisson's ratio nu is %.2f but must be -1 <= nu <= 1./2." % nu
            )

    def _as_consistant_arrays(self, *args):
        """Transforms the inputs into numpy arrays and checks the shape of the
        given inputs. If the shapes are note equal, a ValueError is raised"""
...
class HookesLaw2dPlaneStress(_Hookeslawcore):
    ...
    A cartesian coordinate system is assumed. The stress components in 3
     direction are assumed to be zero, s33 = s13 = s23 = 0."""
    ...
    def strain(self, s11, s22, s12):
        ...
        e11 = 1.0 / self._Et * (s11 - self._nut * s22)
        e22 = 1.0 / self._Et * (s22 - self._nut * s11)
    ...
    def stress(self, e11, e22, g12):
        ...
        factor = self._Et / (1 - np.power(self._nut, 2.0))
...
class HookesLaw2dPlaneStrain(HookesLaw2dPlaneStress):
    ...
    A cartesian coordinate system is assumed. The strain components in
    3 direction are assumed to be zero, e33 = g13 = g23 = 0.
    """
...
class HookesLaw3d(_Hookeslawcore):
    ...
    def strain(self, s11, s22, s33, s12, s13, s23):
        ...
        """
        s11, s22, s33, s12, s13, s23 = self._as_consistant_arrays(
    ...
    def stress(self, e11, e22, e33, g12, g13, g23):
        ...
        s23 : array-like float
            The resulting shear stress component with basis 2-3
        """
        e11, e22, e33, g12, g13, g23 = self._as_consistant_arrays(
