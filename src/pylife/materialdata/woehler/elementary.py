...
class Elementary:
    ...
    def _fit_slope(self):
        slope, lg_intercept, _, _, _ = stats.linregress(np.log10(self._fd.finite_zone.loc[self._fd.finite_zone.fracture == True].load),
                                                        np.log10(self._fd.finite_zone.loc[self._fd.finite_zone.fracture == True].cycles))
    ...
    def _pearl_chain_method(self):
        self._pearl_chain_estimator = PearlChainProbability(self._fd.finite_zone.loc[self._fd.finite_zone.fracture == True], self._slope)
