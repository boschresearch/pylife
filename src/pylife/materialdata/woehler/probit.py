...
class Probit(Elementary):
    def _specific_analysis(self, wc):
        ...
        if len(self._inf_groups) < 2:
            warnings.warn(
                UserWarning(
                    "Probit needs at least two runout load levels. Falling back to Elementary."
                )
            )
            return wc
    ...
    def __probit_rossow_estimation(self):
        ...
        c1 = frac_num == 0
        w = np.where(c1)
        fprobs[w] = 1.0 - 0.5 ** (1.0 / tot_num[w])

        c2 = frac_num == tot_num
        w = np.where(c2)
        fprobs[w] = 0.5 ** (1.0 / tot_num[w])
    ...
    def __probit_analysis(self):
        if self._fd.num_runouts == 0:
            return 1.0, 0.0, self._transition_cycles(0.0)

        fprobs, load = self.__probit_rossow_estimation()

        self._probability_fit = ProbabilityFit(fprobs, load)

        TS = functions.std_to_scattering_range(1.0 / self._probability_fit.slope)
